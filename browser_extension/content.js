(() => {
  ...
  function renderHelpContent(body){
    ...
    const sections=[
      ...
      ['3 · Brain · Vault → Chat','Brain → select exact notes or Load a Context Space → Insert puts their contents in the chat box. Attach .md attempts to attach the actual selected Markdown files.'],
      ['4 · Context Spaces','Select several notes → Save Space → name it. Later choose that Space → Load. Only those exact notes are selected/read.'],
      ['5 · [Note.md] references','Type [Chapter 2.md] in the AI composer. CAIRN offers an explicit resolver; duplicate filenames require an exact path instead of guessing.'],
      ['6 · Agent','Ask naturally: show history · find my SVM notes · what is in [Chapter 2.md]? · list notes · status · doctor · undo last. Ollama is optional.'],
      ['7 · If something goes wrong','Run Dashboard → Doctor. If Bridge is disconnected, verify CAIRN.bat is running and paste the current token again. If notes look stale, press Brain ↻. If a write reports VERSION_CONFLICT, reopen the target and retry—CAIRN did not overwrite the newer file.'],
      ['8 · Requirements','Windows 10/11, Python 3.11+ with pip/venv, and a Chromium browser (Chrome/Brave/Edge) for the extension. Obsidian is optional for core file operations. Ollama is optional for local AI assistance.']
    ];
  ...
  async function renderBrainContent(body){
    ...
    footer(body,saveSpace,insert,attachSelected);
  }

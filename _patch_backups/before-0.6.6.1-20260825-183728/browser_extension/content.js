(() => {
  ...
  assistantButton.title = 'Open CAIRN Brain, Agent, and Help';
  ...
  async function renderNoteViewer(note,backFn=()=>renderAssistant('brain')){
    ...
    try{const r=await message({type:'CAIRN_READ_NOTE',path:note.path});body.querySelector('.cairn-status')?.remove();const meta=document.createElement('div');meta.className='cairn-meta';meta.textContent=`${note.path} · ${r.chars.toLocaleString()} characters`;const pre=document.createElement('pre');pre.className='cairn-md-view';pre.textContent=r.content;body.append(meta,pre);
      ...
      const detach=document.createElement('button');detach.className='cairn-secondary';detach.textContent='Detach .md';detach.onclick=async()=>{const d=await detachMarkdown(note);if(d.ok)status(body,`Detached ${d.count} CAIRN-managed attachment${d.count===1?'':'s'} from this chat.`,'ok');else status(body,'CAIRN could not find that attachment in the current composer. It may already have been removed or the site hides its attachment controls.','bad');};
  ...
  function isVisible(el){if(!el)return false;const style=getComputedStyle(el);const r=el.getBoundingClientRect();return style.display!=='none'&&style.visibility!=='hidden'&&r.width>40&&r.height>20;}
  ...
  function findChatInput(){
    const active=document.activeElement;if(isTextInput(active)&&isVisible(active))return active;
    const host=location.hostname.toLowerCase();
    const providerSelectors=host.includes('claude.ai')?[
    ...
    const selectors=[...providerSelectors,'div[contenteditable="true"][role="textbox"]','textarea[placeholder*="Message" i]','textarea[placeholder*="Ask" i]','textarea','[contenteditable="true"]'];
    const found=[];
    for(const selector of selectors){
      for(const el of document.querySelectorAll(selector)){
        if(!isTextInput(el)||!isVisible(el)||found.includes(el))continue;
        // Avoid search boxes and CAIRN's own controls; favor composer-like elements
        // near the lower portion of the viewport.
        const hint=`${el.getAttribute?.('placeholder')||''} ${el.getAttribute?.('aria-label')||''} ${el.className||''}`.toLowerCase();
        if(/search/.test(hint) && !/message|prompt|reply/.test(hint)) continue;
        found.push(el);
      }
    }
    found.sort((a,b)=>{
      const ar=a.getBoundingClientRect(), br=b.getBoundingClientRect();
      const aScore=ar.bottom+(a.classList?.contains('ProseMirror')?10000:0)+(a.id==='prompt-textarea'?10000:0);
      const bScore=br.bottom+(b.classList?.contains('ProseMirror')?10000:0)+(b.id==='prompt-textarea'?10000:0);
  ...
  function inputText(el){return el instanceof HTMLTextAreaElement||el instanceof HTMLInputElement?(el.value||''):(el.innerText||el.textContent||'');}
  function setContentEditableText(el,text){
    el.focus();
    // ProseMirror (used by Claude) generally reacts more reliably to an editing
    // command than to assigning textContent directly.
    try{
  ...
  function setChatText(el,text){
    el.focus();
    if(el instanceof HTMLTextAreaElement||el instanceof HTMLInputElement){nativeSetter(el,text);el.dispatchEvent(new InputEvent('input',{bubbles:true,inputType:'insertText',data:text}));el.dispatchEvent(new Event('change',{bubbles:true}));el.setSelectionRange?.(text.length,text.length);return true;}
  ...
  function assistantTabs(body,active){
    ...
    const items=[['brain','Brain'],['agent','Agent'],['help','Help']];
  ...
  function renderHelpContent(body){
    ...
    const sections=[
      ...
      ['4 · Multi-Provider Prompt','Type a draft prompt in the current AI composer → Brain → Multi-Provider Prompt → choose All open AI, ChatGPT, Claude, Gemini, or another detected provider. Optionally include selected Brain context and/or attach selected .md files. CAIRN inserts only; it never presses Send.'],
  ...
  async function renderProviderBroadcastBox(body){
    ...
    const contextCheck=document.createElement('input');contextCheck.type='checkbox';
    const contextText=document.createElement('span');contextText.textContent='Include selected Brain context';contextLabel.append(contextCheck,contextText);
    const attachLabel=document.createElement('label');attachLabel.className='cairn-broadcast-toggle';
    const attachCheck=document.createElement('input');attachCheck.type='checkbox';
    const attachText=document.createElement('span');attachText.textContent='Attach selected .md';attachLabel.append(attachCheck,attachText);
    ...
    const detach=document.createElement('button');detach.className='cairn-secondary';detach.textContent='Detach from…';
    ...
    insert.onclick=async()=>{
      const input=findChatInput();const prompt=input?inputText(input).trim():'';
      if(!prompt)return status(body,'Type your prompt in the current AI chat box first. CAIRN will copy that draft to the other provider tabs.','bad');
      ...
      try{
        ...
        status(body,`Inserted into ${r.inserted}/${r.matched} AI tab${r.matched===1?'':'s'}${attachmentNote}. Review each tab, then send normally.`,kind);
      }catch(e){status(body,cleanError(e),'bad');}
    ...
    detach.onclick=async()=>{
      const paths=[...brainSelected.keys()];
      if(!paths.length)return status(body,'Select the Brain notes whose CAIRN-managed attachments you want to detach.','bad');
      insert.disabled=true;detach.disabled=true;
      try{
        status(body,'Detaching selected CAIRN-managed .md files from target tabs…');
  ...
  async function renderBrainContent(body){
    brainSelected=new Map(); panel.classList.add('cairn-brain-mode');
    ...
    const detachSelected=document.createElement('button');detachSelected.className='cairn-secondary';detachSelected.textContent='Detach .md';detachSelected.title='Remove CAIRN-managed selected Markdown attachments from this chat composer';detachSelected.onclick=async()=>{const paths=[...brainSelected.keys()];if(!paths.length)return status(body,'Select the attached notes you want to detach.','bad');detachSelected.disabled=true;try{const r=await detachMarkdownPaths(paths);if(r.ok)status(body,`Detached ${r.count} selected .md attachment${r.count===1?'':'s'} from this chat.`,'ok');else status(body,'CAIRN could not find those attachments in this composer. They may already be removed or the provider may hide its attachment controls.','bad');}catch(e){status(body,cleanError(e),'bad');}finally{detachSelected.disabled=false;}};
  ...
  chrome.runtime.onMessage.addListener((msg,sender,sendResponse)=>{
    ...
    if(msg?.type==='CAIRN_UI_REMOTE_COMPOSE'){
      (async()=>{
        const inserted=!!(msg.text&&insertIntoChat(msg.text));

## CAIRN Fusion 0.6.6.1 — Multi-Provider Prompt experiment

This release keeps the refined 0.6.5 backend and adds a browser-only workflow experiment:

- **Multi-Provider Prompt** inside Brain detects other open supported AI tabs and can insert the current draft prompt into all of them or into a chosen provider such as ChatGPT, Claude, or Gemini. It never auto-sends.
- Optional **Include selected Brain context** prepends the exact selected-note context bundle to the prompt before broadcasting.
- Optional **Attach selected .md** asks each target provider tab to attach the selected Markdown files using CAIRN's existing best-effort upload adapter.
- **Detach .md** removes CAIRN-managed attachments from the current composer; **Detach from…** does the same across the selected provider target. Detach never deletes vault files.
- Additional detected providers include Copilot, Perplexity, Grok, DeepSeek, Poe, Meta AI, Mistral, HuggingChat, You.com, and Pi where the normal CAIRN content script can reach the chat composer.

The feature deliberately targets **other open AI tabs** so the prompt already typed in the current tab is not duplicated there. Existing Brain, Agent, Save Chat, Context Spaces, watcher, SQLite/FTS5, atomic writes, history, and Undo behavior are unchanged from 0.6.5.
...
## 0.6.6.1 Multi-Provider fixes

The unified CAIRN panel now uses four tabs: **Brain → Multi-Provider → Agent → Help**. Multi-Provider prompt broadcasting works with zero selected Brain notes unless Brain context or `.md` attachment is explicitly enabled. ChatGPT/Claude composer detection is more tolerant of compact editors, remote insertion retries safely, selected Multi-Provider options show a visible ✓ state, and Detach removes all CAIRN-managed attachments at once.
